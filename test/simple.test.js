...
import {LWEencrypt, LWEsmul,decryptMatrixToBN, LWEaddMatrix,LWEsetup} from 'nodeseal-bn';
...
describe("unit tests", async function() {
    ...
    it("simple test", async function() {
        ...
        var encPriv = await LWEencrypt(setup.encryptor,key.getPrivate(),setup.encoder);
        ...
        var cipherTex1 = await LWEsmul(setup.evaluator,setup.encoder,encPriv,input2,setup.seal);

        var c2 = await LWEaddMatrix(setup.evaluator,cipherTex00.contents,cipherTex1.contents,seal);
        var dec = await decryptMatrixToBN(setup.decryptor,c2.contents,setup.encoder);
